...
pub fn solveday(day: usize, res: *[25][2]i128) void {
    switch (day) {
        1 => {
...
fn checkResults(res: [25][2]i128) void {
    print("{any}\n", .{res});
...
pub fn mainthreaded() !void {
    ...
    for (0..25) |tid| {
        threads[tid] = std.Thread.spawn(.{}, solveday, .{ tid + 1, &res }) catch unreachable;
    }
    ...
    const threadtime = timer.lap();
    print("Threaded time: {any}\n", .{totime(threadtime)});
    print("Threaded time ns: {}\n", .{threadtime});
}
